...
function zipUp(html) {
  ...
  execSync(`zip -9 -X -q "${ZIP}" index.html`, { cwd: OUT });
  execFileSync(ect, ['-9', '-zip', '-strip', ZIP], { stdio: 'ignore' });
  execFileSync(advzip, ['-z', '-4', '-q', ZIP], { stdio: 'ignore' });
  return statSync(ZIP).size;
}

if (DEV) {
  mkdirSync(OUT, { recursive: true });
  writeFileSync(OUT + '/index.html', shell('', true));
  ...
  console.log(`\n  PRISM LOOP dev  ->  http://${hosts[0]}:${port}\n`);
} else {
